...
export default function Editvehicules({params}){
  ...
  async function fetchvehicules(){ const {data}=await supabase.from('vehiculess').select('*').eq('id',params.id).single(); if(data){ setName(data.name); setDescription(data.description); setPrice(data.price); } }
  async function updatevehicules(e){ e.preventDefault(); await supabase.from('vehiculess').update({name,description,price}).eq('id',params.id); router.push('/admin/dashboard'); }
  return (
    <form onSubmit={updatevehicules} className="max-w-md mx-auto mt-10">
